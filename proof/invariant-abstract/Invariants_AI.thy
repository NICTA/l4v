  ...
  "tcb_cap_cases \<equiv>
   [tcb_cnode_index 0 \<mapsto> (tcb_ctable, tcb_ctable_update, (\<lambda>_ _. \<top>)),
    ...
    tcb_cnode_index 3 \<mapsto> (tcb_fault_handler, tcb_fault_handler_update,
                             (\<lambda>_ _. is_ep_cap or (op = NullCap))),
...
lemma ran_tcb_cap_cases:
  "ran (tcb_cap_cases) =
    {(tcb_ctable, tcb_ctable_update, (\<lambda>_ _. \<top>)),
     ...
     (tcb_fault_handler, tcb_fault_handler_update, (\<lambda>_ _. is_ep_cap or (op = NullCap))),
     (tcb_timeout_handler, tcb_timeout_handler_update, (\<lambda>_ _. is_ep_cap or (op = NullCap)))}"
